...
class WhatPressure extends WhatBase {
  ...
  hidden var dataOneMin = [];
  hidden var dataPerMin = [];
  ...
  function getAvgPressureOneMin() {
    ...
    var ppx = 0.0f;
    for (var x = 0; x < dataOneMin.size(); x++) {
      ppx = ppx + dataOneMin[x];
    }
    return ppx / dataOneMin.size();
  }
  ...
  function pressurePerMinX() {
    ...
    var ppx = 0.0f;
    for (var x = 0; x < dataPerMin.size(); x++) {
      ppx = ppx + dataPerMin[x];
    }
    return ppx / dataPerMin.size();
  }
  ...
  // barometric pressure rises or falls more than 0.18 in-Hg in less than three
  // hours,
